...
env-supabase: ## Generate .env from Supabase CLI (run after: supabase start)
	...
	url=$$(echo "$$json" | jq -r '.API_URL // .APIUrl // .api_url // empty'); \
	key=$$(echo "$$json" | jq -r '.SERVICE_ROLE_KEY // .service_role_key // empty'); \
	jwt=$$(echo "$$json" | jq -r '.JWT_SECRET // .jwt_secret // empty'); \
	if [ -z "$$url" ] || [ -z "$$key" ] || [ -z "$$jwt" ]; then \
	  echo "Run 'supabase start' first, then try again. Or run 'supabase status --output json' and set .env manually."; exit 1; \
	fi; \
